        ...
        <main className="min-h-screen pt-20 md:pt-24 bg-background">
            <Container className="py-12 md:py-24">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {BLOG_POSTS.map((post) => (
                        <div key={post.id} className="group flex flex-col bg-card border border-border rounded-xl overlow-hidden hover:border-secondary/50 transition-all duration-300">
                            <div className={`h-48 w-full ${post.image} relative overflow-hidden rounded-t-xl`}>
                                <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors" />
                                <Badge className="absolute top-4 right-4 bg-background/80 backdrop-blur text-secondary border-secondary/20 hover:bg-background/90">
                                    {post.category}
                                </Badge>
                            </div>
                            ...
                            <div className="p-6 flex flex-col flex-grow">
                                ...
                                <div className="flex items-center justify-between mt-auto pt-4 border-t border-border/50">
                                    <span className="flex items-center text-sm text-muted-foreground">
                                        <User className="h-3 w-3 mr-2 text-secondary" /> {post.author}
                                    </span>
                                    <Button variant="link" className="px-0 text-secondary group-hover:text-secondary/80">
